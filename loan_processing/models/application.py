...
class LoanApplication(BaseModel):
    ...
    date_of_birth: datetime = Field(description="Date of birth for age verification")

    # Loan details
    loan_amount: Decimal = Field(description="Requested loan amount in USD", gt=0, max_digits=10, decimal_places=2)
    ...
    annual_income: Decimal = Field(description="Annual income in USD", ge=0, max_digits=10, decimal_places=2)

    employment_status: EmploymentStatus = Field(description="Current employment status")

    employer_name: str | None = Field(None, description="Name of current employer", max_length=100)

    months_employed: int | None = Field(None, description="Months at current job", ge=0)

    # Optional financial details
    monthly_expenses: Decimal | None = Field(
        None, description="Monthly expenses in USD", ge=0, max_digits=8, decimal_places=2
    )

    existing_debt: Decimal | None = Field(
        None, description="Total existing debt in USD", ge=0, max_digits=10, decimal_places=2
    )

    assets: Decimal | None = Field(None, description="Total assets in USD", ge=0, max_digits=12, decimal_places=2)

    down_payment: Decimal | None = Field(
        None, description="Down payment amount in USD", ge=0, max_digits=10, decimal_places=2
    )
