...
class BaseAssessment(BaseModel):
    ...
    assessed_by: str = Field(
        description="Agent or system that performed assessment"
    )
...
class CreditAssessment(BaseAssessment):
    ...
    credit_score: int = Field(
        description="Verified credit score",
        ge=300,
        le=850
    )
    
    credit_bureau: str = Field(
        description="Source credit bureau (Experian, Equifax, TransUnion)"
    )
    
    # Credit history details
    payment_history_score: float = Field(
        description="Payment history assessment score",
        ge=0.0,
        le=1.0
    )
    ...
    bankruptcies: int = Field(
        description="Number of bankruptcies on record",
        ge=0
    )
    
    # Assessment results
    risk_level: RiskLevel = Field(
        description="Assessed credit risk level"
    )
    
    recommendation: str = Field(
        description="Credit assessment recommendation"
    )
    
    red_flags: list[str] = Field(
        default_factory=list,
        description="Identified credit risk factors"
    )
...
class IncomeVerification(BaseAssessment):
    ...
    # Income verification
    stated_income: Decimal = Field(
        description="Income stated by applicant",
        ge=0,
        max_digits=10,
        decimal_places=2
    )
    
    verified_income: Decimal = Field(
    ...
    verification_method: str = Field(
        description="Method used for verification (paystub, tax_return, employer_contact)"
    )
    ...
    employer_contact_verified: bool = Field(
        description="Whether employer contact information was verified"
    )
    
    employment_start_date: datetime | None = Field(
        None,
        description="Employment start date if verified"
    )
    
    # Income analysis
    income_stability_score: float = Field(
        description="Income stability assessment score",
        ge=0.0,
        le=1.0
    )
    
    income_trend: str = Field(
        description="Income trend analysis (increasing, stable, declining)"
    )
    
    # Discrepancy analysis
    discrepancy_amount: Decimal = Field(
        description="Absolute difference between stated and verified income",
        ge=0,
        max_digits=10,
        decimal_places=2
    )
    ...
    discrepancy_percentage: float = Field(
        description="Percentage discrepancy in income",
        ge=0.0
    )
    
    verification_issues: list[str] = Field(
        default_factory=list,
        description="Issues encountered during verification"
    )
...
class RiskAssessment(BaseAssessment):
    ...
    credit_risk_score: float = Field(
        description="Credit-related risk component",
        ge=0.0,
        le=1.0
    )
    
    income_risk_score: float = Field(
        description="Income-related risk component",
        ge=0.0,
        le=1.0
    )
    
    employment_risk_score: float = Field(
        description="Employment-related risk component",
        ge=0.0,
        le=1.0
    )
    
    debt_risk_score: float = Field(
        description="Debt-related risk component",
        ge=0.0,
        le=1.0
    )
    
    loan_specific_risk_score: float = Field(
        description="Loan-specific risk factors",
        ge=0.0,
        le=1.0
    )
    ...
    mitigating_factors: list[str] = Field(
        default_factory=list,
        description="Risk mitigating factors"
    )
    
    # Recommendations
    approval_recommendation: bool = Field(
        description="Whether to approve the loan"
    )
    
    recommended_amount: Decimal | None = Field(
        None,
        description="Recommended loan amount if different from requested",
        ge=0,
        max_digits=10,
        decimal_places=2
    )
    ...
    required_conditions: list[str] = Field(
        default_factory=list,
        description="Required conditions for approval"
    )
...
class ComprehensiveAssessment(BaseModel):
    ...
    credit_assessment: CreditAssessment | None = Field(
        None,
        description="Credit assessment results"
    )
    
    income_verification: IncomeVerification | None = Field(
        None,
    ...
    agents_involved: list[str] = Field(
        default_factory=list,
        description="List of agents that participated in assessment"
    )
    ...
    overall_confidence: float | None = Field(
        None,
        description="Overall confidence score across all assessments",
        ge=0.0,
        le=1.0
    )
