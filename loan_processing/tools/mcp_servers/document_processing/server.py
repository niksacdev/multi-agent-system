...
logger.info("Document Processing MCP Server initialized", 
...
async def extract_text_from_document(document_path: str, document_type: str = "auto") -> str:
    ...
    """
    logger.info("Document text extraction request", 
               document_path=document_path, document_type=document_type, component="mcp_server")
...
async def classify_document_type(document_content: str) -> str:
    ...
    logger.info("Document classification request", 
               content_length=len(document_content), component="mcp_server")
...
async def validate_document_format(document_path: str, expected_format: str) -> str:
    ...
    logger.info("Document validation request", 
               document_path=document_path, expected_format=expected_format, component="mcp_server")
...
async def extract_structured_data(document_path: str, data_schema: str) -> str:
    ...
    """
    logger.info("Structured data extraction request", 
               document_path=document_path, component="mcp_server")
...
async def convert_document_format(input_path: str, output_format: str) -> str:
    ...
    """
    logger.info("Document conversion request", 
               input_path=input_path, output_format=output_format, component="mcp_server")
...
if __name__ == "__main__":
    ...
    if transport == "sse":
        logger.info("Starting Document Processing MCP Server", 
                   transport="sse", url="http://localhost:8011/sse", component="mcp_server")
    else:
        logger.info("Starting Document Processing MCP Server", 
