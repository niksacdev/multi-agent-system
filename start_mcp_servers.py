...
from loan_processing.utils import get_logger, correlation_context  # noqa: E402

# Initialize logging
logger = get_logger(__name__)

servers = [
    {"module": "loan_processing.tools.mcp_servers.application_verification.server", "port": 8010, "name": "Application Verification"},
    {"module": "loan_processing.tools.mcp_servers.document_processing.server", "port": 8011, "name": "Document Processing"},
    {"module": "loan_processing.tools.mcp_servers.financial_calculations.server", "port": 8012, "name": "Financial Calculations"},
]
...
def start():
    ...
    with correlation_context("mcp_server_startup") as session_id:
        logger.info("Starting MCP server startup sequence", 
        ...
        if started_servers:
            logger.info("Started servers summary", 
                       servers=[s["name"] for s in started_servers],
                       component="server_manager")

        if failed_servers:
            logger.warning("Failed servers summary", 
